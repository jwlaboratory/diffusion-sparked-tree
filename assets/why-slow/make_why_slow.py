...
def dflash():
    ...
    node(ax, 5.85, 2.0, 2.2, 1.25, "Top-K", sub="argmax", edge=BLUE, textcolor=INK)
...
def dspark():
    ...
    container(ax, 0.4, 0.65, 12.5, 3.55, "GPU", GPU_FILL, GPU_EDGE)

    node(ax, 2.15, 2.05, 2.6, 1.25, "Diffusion\ndrafter", edge=BLUE, textcolor=INK)
    node(ax, 5.35, 2.05, 2.6, 1.25, "Autoregressive\nMarkov head", edge=BLUE, textcolor=INK)
    node(ax, 8.25, 2.05, 1.9, 1.25, "Top-K", edge=BLUE, textcolor=INK)
    node(ax, 11.0, 2.05, 2.1, 1.25, "Sampler", edge=BLUE, textcolor=INK)

    arrow(ax, 3.45, 2.05, 4.05, 2.05, color=BLUE)   # drafter -> markov head
    arrow(ax, 6.65, 2.05, 7.30, 2.05, color=BLUE)   # markov head -> top-K
    arrow(ax, 9.20, 2.05, 9.95, 2.05, color=BLUE)   # top-K -> sampler

    # iterative loop over head + top-K + sampler
    span_loop(ax, 5.35, 11.0, 2.68, BLUE,
              "loop: N small GPU ops (Markov head + Top-K + sample)")

    # sampler -> output (crosses GPU boundary)
    arrow(ax, 12.05, 2.05, 13.85, 2.05, color=INK2)
    node(ax, 14.9, 2.05, 1.9, 1.25, "Output", edge=INK, textcolor=INK)
...
def sparklingtree():
    ...
    node(ax, 9.85, 2.05, 2.3, 1.35, "Top-K", edge=AQUA, textcolor=INK)
    node(ax, 12.9, 2.05, 2.7, 1.35, "Markov\nmodel", edge=AQUA, textcolor=INK)

    arrow(ax, 8.0, 2.05, 8.70, 2.05, color=AQUA)     # heap -> top-K
    arrow(ax, 11.0, 2.05, 11.55, 2.05, color=AQUA)   # top-K -> markov

    # loop over the whole CPU pipeline, recomputed on every pop
    span_loop(ax, 6.75, 12.9, 2.78, AQUA,
              "loop: every pop (Top-K + Markov recomputed on CPU)", rad=0.26)

    # CPU -> output
    arrow(ax, 14.25, 2.05, 15.85, 2.05, color=INK2)
