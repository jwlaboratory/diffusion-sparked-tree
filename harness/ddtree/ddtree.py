...
DDTREE_STAGE_ORDER = ("draft", "tree_build", "tree_compile", "verify", "walk_accept", "kv_update", "state_carry")
...
def build_ddtree_tree(
    ...
    # 2. dict lookups, dynamic branching etc, much better handled by CPU
   # Note: Modern state-of-the-art speculative inference engines do sometimes write custom CUDA kernels (like parallel speculative search or prefix trees) to avoid CPU sync entirely, but it requires writing complex lower-level C++/CUDA code instead of clean, maintainable Python.
