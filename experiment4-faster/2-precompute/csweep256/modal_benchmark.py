...
FAST_CS = [128, 256, 512, 1024, 2048]   # fast builder is ~linear in C
PRECOMPUTE_CS = [128, 256, 512, 1024]   # precompute is O(L C^2 R): stop at 1024
...
METHODS = (
    # Acceptance ceiling: exact best-first (no restriction, no tree_kwargs).
...
CACHE_DIR = "/results/csweep256/cache"
...
# csweep256/ -> 2-precompute/ -> experiment4-faster/ -> repo root; harness lives at root.
HARNESS_DIR = HERE.parent.parent.parent / "harness"
...
app = modal.App("ddtree-exp4-csweep256")
...
def run_experiment(cfg: dict) -> dict:
    ...
    out = Path("/results/csweep256/summary.json")
...
def main(smoke: bool = False, spawn: bool = False):
    ...
    if smoke:
        ...
        cfg["tree_budgets"] = [256]
        cfg["warmup_tokens"] = 32

    if spawn:
        call = run_experiment.spawn(cfg)
        print(f"spawned: {call.object_id}")
        print("progress:  modal volume ls ddtree-results csweep256/cache")
        print("fetch:     modal volume get ddtree-results csweep256/summary.json")
        return
