...
class Method:
    ...
    # Experiment 4 additions.
    measure_confidence: bool = False  # log per-round drafter confidence (free)
    gate: Optional[list] = None       # [threshold, small_budget]; shrink when CONFIDENT
    head: Optional[str] = None        # key into heads; None = free pred_chain_len
...
def load_acceptance_heads(spec: dict, device) -> dict[str, object]:
    """Load trained TreeAcceptanceHead checkpoints, keyed by name.

    spec maps head name -> path. Loading is strict: a missing file raises rather
    than degrading to the free estimator, because "trained head" vs "free
    estimator" is the comparison this experiment exists to make.
    """
    ...
    for name, path in (spec or {}).items():
        ...
        head = TreeAcceptanceHead(hidden=blob.get("hidden", 32))
        head.load_state_dict(blob["state_dict"])
        head.eval().to(device)
        for p in head.parameters():
            p.requires_grad_(False)
        heads[name] = head
        print(f"[head] {name} <- {path} (val MAE {blob.get('val_mae', float('nan')):.3f} tokens)",
              flush=True)
    return heads
...
def build_method_callable(
    ...
    if method.verify == "tree":
        # The gate is a hard [threshold, small_budget] pair rather than a fraction so
        # the shrunk width is exactly what gets reported, with no rounding to explain.
        gate = tuple(method.gate) if method.gate else None
        if gate is not None and gate[1] > tree_budget:
            raise ValueError(
                f"method {method.name!r}: gate budget {gate[1]} exceeds tree_budget "
                f"{tree_budget} -- the gate must SHRINK the tree, never grow it")
        acc_head = None
        if method.head is not None:
            if method.head not in heads:
                raise ValueError(f"method {method.name!r}: unknown head {method.head!r}")
            acc_head = heads[method.head]
        return lambda ids: sparked_tree_generate(
            ...
            measure_confidence=method.measure_confidence,
            confidence_gate=gate, tree_acceptance_head=acc_head,
            **common,
        )
...
def run(cfg: dict, on_checkpoint=None) -> dict:
    ...
    # Trained tree-acceptance heads, keyed by name. A method referencing one it did
    # not get is a hard error in build_method_callable rather than a silent fallback
    # to the free estimator -- those two arms are the comparison, so quietly
    # substituting one for the other would invalidate the result.
    heads = load_acceptance_heads(cfg.get("acceptance_heads", {}), device)

    fns_by_budget: dict[int, dict[str, Callable]] = {}
    for budget in cfg["tree_budgets"]:
        for m in methods:
            fn = build_method_callable(
                m, backbones, correctors, target, tokenizer.eos_token_id, warmup_cfg, budget, heads
            )
            _ = fn(warmup_ids)
        fns_by_budget[budget] = {
            m.name: build_method_callable(
                m, backbones, correctors, target, tokenizer.eos_token_id, cfg, budget, heads
            )
            for m in methods
        }
    ...
    for budget in cfg["tree_budgets"]:
        ...
        for dataset_name, max_samples in cfg["tasks"]:
            ...
            if lengths is None:
                ...
                # Per-unit payload. `acc` and `conf` are appended in lockstep so
                # index i is the same round in both -- that pairing is the whole
                # point of the instrumentation (investigate/FINDINGS.md).
                lengths = {m.name: {"acc": [], "conf": [], "gate_budgets": [],
                                    "tpot": [], "decode_time": [], "out_tokens": []}
                           for m in methods}
                for row in dataset:
                    ...
                    for m in methods:
                        ...
                        rec = lengths[m.name]
                        rec["acc"].extend(int(a) for a in result.acceptance_lengths)
                        rec["conf"].extend(getattr(result, "confidence_by_round", []) or [])
                        rec["gate_budgets"].extend(getattr(result, "gate_budgets", []) or [])
                        # This core reports tpot and token count but no decode
                        # total, so derive it rather than depend on a field that
                        # only exists in the exp3 harness.
                        tpot = float(result.time_per_output_token)
                        n_out = int(result.num_output_tokens)
                        rec["tpot"].append(tpot)
                        rec["decode_time"].append(tpot * n_out)
                        rec["out_tokens"].append(n_out)
                if cpath:
            ...
            for m in methods:
                ...
                if rec["tpot"]:
                    e["tpot_ms"] = 1000.0 * mean(rec["tpot"])
                    e["decode_time_s"] = sum(rec["decode_time"])
                if rec["gate_budgets"]:
                    g = rec["gate_budgets"]
                    small = min(g)
                    e["gated_frac"] = sum(1 for b in g if b == small) / len(g)
                    e["mean_budget"] = mean(g)
